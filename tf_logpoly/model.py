...
class TFLogpoly:
    ...
    def __init__(self, factor_degree, factors_count=1):
        ...
        self.theta = np.array([])
        self.SS = tf.placeholder(tf.float32, [self.factor_degree])
        self.SS = tf.placeholder(tf.int32, [1])
    ...
    def newton_step(self, theta):
        ...
        if compute_log_likelihood(SS, np.concatenate(
                [theta.reshape([-1, k + 1]), (theta_new + lam * delta_theta).reshape([1, -1])]),
                                  n) <= current_log_likelihood:
