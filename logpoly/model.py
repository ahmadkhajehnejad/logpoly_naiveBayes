...
class Logpoly:
    ...
    def fit_new_factor(self):
        ...
        [logZ, new_critical_points] = log_integral_exp(compute_poly, np.concatenate([theta.reshape([-1,k]), theta_new.reshape([1,-1])]), critical_points, return_new_critical_points=True)
        return [theta_new, logZ, new_critical_points]
    
    
    
    def fit_logpoly(self):
        self.theta = np.array([])
        ...
        for i in range(config.num_factors):
            ...
            self.theta = np.concatenate([self.theta.reshape([-1,config.factor_degree]), theta_new.reshape([1,-1])])